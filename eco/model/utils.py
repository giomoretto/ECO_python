...
def check_validity(x: np.ndarray, ca: float, par_model: Any, par_op: Any) -> np.ndarray:
    """Check and limit states to physical boundaries
    ...
    # pCyl
    eps = 1e-3
    v_cyl, _, _ = cyl_vol(ca, par_model.eng)
    lb = 0.8 * par_op.p_int * (par_op.v_int / v_cyl)**1.4
    p_cyl = 0.5 * (p_cyl - lb) + 0.5 * np.sqrt(eps**2 + (p_cyl - lb)**2) + lb
    ...
    if en_nox:
        eps = 1e-10
        lb = 0
        nox = 0.5 * (nox - lb) + 0.5 * np.sqrt(eps**2 + (nox - lb)**2) + lb
    
    # Create output vector
    x_out = np.array([p_cyl, q_comb, imep])
    if en_nox:
        x_out = np.array([p_cyl, q_comb, imep, t_uz, nox])
