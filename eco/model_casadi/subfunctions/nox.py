"""NOx model helper functions (CasADi version)"""
...
def two_zone_model(q_comb, theta_uz0, dp_cyl, p_cyl, dv_cyl, v_cyl,
                   t_cyl, kappa, m_tot, par_op, par_model, x_bg, x_bz):
    """Two-zone model for calculating burned gas temperature (CasADi symbolic).
...
def _nox_eq_mole_fractions(theta_bz):
    """Equilibrium mole fractions via polynomial regression (CasADi).

    Args:
        theta_bz: Burned zone temperature [K]

    Returns:
        x_n, x_o, x_n2, x_o2, x_oh, x_no: Equilibrium mole fractions
    """
    def _poly_exp(theta, coefs):
        regr = ca.vertcat(1, theta, theta**2, theta**3)
        return ca.exp(ca.dot(coefs, regr))

    x_n  = _poly_exp(theta_bz, np.array([-93.5994804475774, 0.0635582925091314,
                                          -1.78636544365512e-05, 1.86389452885879e-09]))
    x_o  = _poly_exp(theta_bz, np.array([-63.6851353343809, 0.0451581256769548,
                                          -1.25158990443408e-05, 1.27610634104909e-09]))
    x_n2 = _poly_exp(theta_bz, np.array([-0.216167752677653, -0.000141506777271925,
                                           8.06135838153914e-08, -1.54929244598273e-11]))
    x_o2 = _poly_exp(theta_bz, np.array([-33.6431381400466, 0.0227657004453811,
                                          -6.04189495663701e-06, 5.68577263026475e-10]))
    x_oh = _poly_exp(theta_bz, np.array([-38.9129885343657, 0.0269739906905952,
                                          -7.40667321236856e-06, 7.42212500799537e-10]))
    x_no = _poly_exp(theta_bz, np.array([-34.406106632447, 0.0234949344561571,
                                          -6.41287032472529e-06, 6.35111932336585e-10]))
    return x_n, x_o, x_n2, x_o2, x_oh, x_no


def nox_model(q_comb, theta_uz, theta_bz, no, m_tot,
              par_op, par_model, x_bg,
              v_cyl, dv_cyl, p_cyl, dp_cyl, theta_cyl):
    """NOx formation rate using extended Zeldovich mechanism (CasADi symbolic).
    ...
    k1f = 1.47e13 * theta_bz_sat**0.3 * ca.exp(-37885.8746 / theta_bz_sat)
    k2f = 6.40e09 * theta_bz_sat**1.0 * ca.exp(-3163.169283 / theta_bz_sat)
    k3f = 3.80e13

    x_n, x_o, x_n2, x_o2, x_oh, x_no = _nox_eq_mole_fractions(theta_bz_sat)
    no_eq = x_no * mf2c
